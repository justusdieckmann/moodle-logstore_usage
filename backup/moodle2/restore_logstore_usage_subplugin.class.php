...
/**
 * Restore implementation for the (tool_log) logstore_usage subplugin.
 *
...
class restore_logstore_usage_subplugin extends restore_tool_log_logstore_subplugin {
    ...
    protected function define_logstore_subplugin_structure() {
        ...
        if (!in_array('logstore_usage', $enabledlogstores)) {
        ...
        $elepath = $this->get_pathfor('/logstore_usage_log');
    ...
    /**
     * Process logstore_usage_log entries.
     *
     ...
     */
    public function process_logstore_usage_log($data) {
        global $DB;

        $data = $this->process_log($data);

        if ($data) {
            $DB->insert_record('logstore_usage_log', $data);
        }
    }
}

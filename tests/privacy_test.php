...
class logstore_usage_privacy_testcase extends provider_testcase {
    ...
    public function test_get_contexts_for_userid() {
        ...
        $e = \logstore_usage\event\unittest_executed::create(['context' => $cm1ctx]);
        $e->trigger();
        ...
        $e = \logstore_usage\event\unittest_executed::create(['context' => $cm2ctx, 'relateduserid' => $u2->id]);
        $e->trigger();
        ...
        $e = \logstore_usage\event\unittest_executed::create(['context' => $c2ctx, 'relateduserid' => $u3->id]);
        $e->trigger();
    ...
    public function test_add_userids_for_context() {
        ...
        $userlist = new \core_privacy\local\request\userlist($sysctx, 'logstore_usage_log');
        ...
        $this->setUser($u1);
        $e = \logstore_usage\event\unittest_executed::create(['context' => $sysctx]);
        $e->trigger();
        // User two (userids) and three (relateduserid) should be added.
        $this->setUser($u2);
        $e = \logstore_usage\event\unittest_executed::create(['context' => $sysctx, 'relateduserid' => $u3->id]);
        $e->trigger();
    ...
    public function test_delete_data_for_user() {
        ...
        $e = \logstore_usage\event\unittest_executed::create(['context' => $c1ctx]);
        $e->trigger();
        $e = \logstore_usage\event\unittest_executed::create(['context' => $c2ctx]);
        $e->trigger();
        ...
        $this->assertEquals(3, $DB->count_records('logstore_usage_log', ['userid' => $u1->id]));
    ...
    public function test_delete_data_for_userlist() {
        ...
        $userlist = new \core_privacy\local\request\approved_userlist($sysctx, 'logstore_usage_log', [$u1->id, $u3->id]);
        ...
        $records = $DB->get_records('logstore_usage_log', ['contextid' => $sysctx->id]);
    ...
    public function test_export_data_for_user() {
        ...
        // User 1 is the author.
        $this->setUser($u1);
        $e = \logstore_usage\event\unittest_executed::create(['context' => $c1ctx, 'other' => ['i' => 0]]);
        $e->trigger();

        // User 2 is related.
        $this->setUser(0);
        $e = \logstore_usage\event\unittest_executed::create(['context' => $c1ctx, 'relateduserid' => $u2->id,
            'other' => ['i' => 1]]);
        $e->trigger();
        ...
        $e = \logstore_usage\event\unittest_executed::create(['context' => $c1ctx, 'relateduserid' => $u4->id,
            'other' => ['i' => 2]]);
        $e->trigger();
        ...
        writer::reset();
        provider::export_user_data(new approved_contextlist($u2, 'logstore_usage', [$c2ctx->id, $c1ctx->id]));
        $data = writer::with_context($c2ctx)->get_data($path);
        $this->assertEmpty($data);
        $data = writer::with_context($c1ctx)->get_data($path);
        $this->assertCount(1, $data->logs);
        ...
        writer::reset();
        provider::export_user_data(new approved_contextlist($u3, 'logstore_usage', [$c2ctx->id, $c1ctx->id]));
        $data = writer::with_context($c2ctx)->get_data($path);
        $this->assertEmpty($data);
        $data = writer::with_context($c1ctx)->get_data($path);
        $this->assertCount(1, $data->logs);
        ...
        writer::reset();
        provider::export_user_data(new approved_contextlist($u4, 'logstore_usage', [$c2ctx->id, $c1ctx->id]));
        $data = writer::with_context($c2ctx)->get_data($path);
        $this->assertEmpty($data);
        $data = writer::with_context($c1ctx)->get_data($path);
        $this->assertCount(1, $data->logs);
        ...
        $this->setUser($u1);
        $e = \logstore_usage\event\unittest_executed::create(['context' => $c2ctx, 'relateduserid' => $u2->id,
            'anonymous' => true]);
        $e->trigger();
        $this->setAdminUser();
        \core\session\manager::loginas($u3->id, $sysctx);
        $e = \logstore_usage\event\unittest_executed::create(['context' => $c2ctx, 'relateduserid' => $u4->id,
            'anonymous' => true]);
        $e->trigger();

        // Confirm data present for u1.
        provider::export_user_data(new approved_contextlist($u1, 'logstore_usage', [$c2ctx->id]));
        $data = writer::with_context($c2ctx)->get_data($path);
        $this->assertCount(1, $data->logs);
        ...
        // Confirm data present for u2.
        writer::reset();
        provider::export_user_data(new approved_contextlist($u2, 'logstore_usage', [$c2ctx->id]));
        $data = writer::with_context($c2ctx)->get_data($path);
        $this->assertCount(1, $data->logs);
        $this->assertEquals(transform::yesno(true), $data->logs[0]['action_was_done_anonymously']);
        ...
        // Confirm data present for u3.
        writer::reset();
        provider::export_user_data(new approved_contextlist($u3, 'logstore_usage', [$c2ctx->id]));
        $data = writer::with_context($c2ctx)->get_data($path);
        $this->assertCount(1, $data->logs);
        $this->assertEquals(transform::yesno(true), $data->logs[0]['action_was_done_anonymously']);
        $this->assertEquals(transform::yesno(true), $data->logs[0]['author_of_the_action_was_you']);
        ...
        // Confirm data present for u4.
        writer::reset();
        provider::export_user_data(new approved_contextlist($u4, 'logstore_usage', [$c2ctx->id]));
        $data = writer::with_context($c2ctx)->get_data($path);
        $this->assertCount(1, $data->logs);
        $this->assertEquals(transform::yesno(true), $data->logs[0]['action_was_done_anonymously']);
        $this->assertArrayNotHasKey('author_of_the_action_was_you', $data->logs[0]);
